...
create table shopify_stub_products (
...
create index shopify_stub_products_collection_idx on shopify_stub_products using gin (collection_ids);
create index shopify_stub_products_active_idx on shopify_stub_products (created_at desc) where status = 'ACTIVE' and sold_at is null;
create index shopify_stub_products_tags_idx on shopify_stub_products using gin (tags);

-- The "Shopify side" of discount codes (price rules + codes in real Shopify).
create table shopify_stub_discount_codes (
  id uuid primary key default gen_random_uuid(),
...
create index shopify_stub_discount_codes_customer_idx on shopify_stub_discount_codes (customer_gid);

-- Audit log of every connector operation. Drives the demo's "see the
-- integration in action" admin view. Goes away in production.
create table shopify_stub_log (
  id uuid primary key default gen_random_uuid(),
...
create index shopify_stub_log_created_idx on shopify_stub_log (created_at desc);
create index shopify_stub_log_operation_idx on shopify_stub_log (operation, created_at desc);
